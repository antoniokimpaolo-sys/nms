...
async function testSNMP(){const f=new FormData($('#device-form'));const result=$('#snmp-result');result.textContent='Testing...';try{const r=await api('/api/snmp/test',{method:'POST',body:JSON.stringify({ip:f.get('ip'),community:f.get('community')||'public'})});result.textContent=r.ok?'SNMP OK':'SNMP failed: '+(r.error||'timeout');result.style.color=r.ok?'var(--green)':'var(--red)';}catch(err){result.textContent='Test error';result.style.color='var(--red)';}}
